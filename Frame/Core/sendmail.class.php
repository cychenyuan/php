...
class sendmail{
	...
	//$subject 表示邮件标题 
	//$body表示邮件正文
	function postmail($to,$subject = '',$body = '', $sender = '', $receive= ''){
	    ...
	    $mail->Host       = 'smtp.163.com';		// SMTP 服务器(发送服务器)
	    $mail->Port       = 25;					// SMTP服务器的端口号，QQ服务器对应是465

	    // SMTP服务器用户名，也就是发送账户
	    $mail->Username   = 'm15533083721_1@163.com';
	    $mail->Password   = '15027916523fie';       // 客户端授权码，注意，这里不是邮箱密码

	    //回复时回复到这个邮箱，通常跟发送账户那个邮箱一致
	    $mail->SetFrom('m15533083721_1@163.com', $sender);
	    //$mail->AddReplyTo('tang_fp@sina.cn','who');    //如果没有上一个，则回复到这个邮箱

		//$to:要发送到的email地址，也就是收件地址
		//$name: 要显示的名字
	    $mail->AddAddress($to, '$receive');
